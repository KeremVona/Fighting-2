...
const StrategyCard: React.FC<StrategyCardProps> = ({
  ...
  const cardStyle =
    "absolute inset-0 backface-hidden rounded-xl p-6 transition-transform duration-700 overflow-hidden";
  ...
  return (
    <div className="group w-full max-w-sm" style={{ perspective: "1000px" }}>
      <div className="relative h-64 w-full transition-transform duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
        <div
          className={`${cardStyle} bg-slate-900 border border-slate-700 shadow-lg group-hover:border-sky-500/50`}
        >
          <img
            src={three}
            alt="background"
            className="absolute inset-0 h-full w-full object-cover opacity-100 mix-blend-overlay"
          />
        ...
        <div
          className={`${cardStyle} bg-sky-900/50 border border-sky-500 shadow-2xl [transform:rotateY(180deg)]`}
        >
          <div className="h-1 w-full bg-gradient-to-r from-sky-300 to-sky-700" />

          <div className="mt-5">
            <h4 className="text-lg font-bold text-white mb-3">{strategy}</h4>
